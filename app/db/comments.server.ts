...
/** A comment can mention 0..N records, comments and/or mental-state records. */
...
/**
 * Build a comment's typed mention list from the group_concat CSV columns.
 * Shared by every read path (home / notes / search / calendar / export) so they
 * all decode mentions the same way. `mentalCsv` is optional so read paths that
 * don't surface mental mentions can omit it.
 */
export function buildMentions(
  recordCsv: string | null,
  commentCsv: string | null,
  mentalCsv: string | null = null,
): MentionRef[] {
  return [
...
/** The referenced record ids among a mention list (other refs dropped). */
export function recordIdsOf(mentions: MentionRef[]): number[] {
  return mentions.filter((m) => m.kind === "record").map((m) => m.id);
}

/** The referenced mental-state ids among a mention list. */
export function mentalIdsOf(mentions: MentionRef[]): number[] {
  return mentions.filter((m) => m.kind === "mental").map((m) => m.id);
}
...
const insertMentalMentionStmt = db.prepare(
  `INSERT OR IGNORE INTO comment_mental_mentions (comment_id, target_mental_id) VALUES (?, ?)`,
);
