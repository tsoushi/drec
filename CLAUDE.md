...
- **DB 層** (`app/db/`): `*.server.ts` 命名必須（better-sqlite3 をクライアントバンドルに混入させない。`vite.config.ts` の `ssr.external` にも指定済み）。
  ...
  - `records.server.ts` / `comments.server.ts` / `mentals.server.ts` — prepared statement による型付き CRUD。コメントは `comment_mentions` / `comment_comment_mentions` / `comment_mental_mentions`（いずれも多対多）で 0..N 件の記録・コメント・メンタル状態を参照。メンタル状態は独立したタイムライン項目で `score`（REAL、-10〜10・小数可）＋記録日時＋誤差（分）を持つ。
...
- **タイムライン**: 記録（`taken_at`）・コメント（`commented_at`）・メンタル状態（`recorded_at`）をクライアントでマージして新しい順に表示。
